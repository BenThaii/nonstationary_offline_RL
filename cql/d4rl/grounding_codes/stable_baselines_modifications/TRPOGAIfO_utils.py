...
class SegmentGenerator(object):
    def __init__(self, policy, env, horizon, discriminator, explore_discriminator=None, replay_buffer=None, miner=None, entropy_coeff=0.001, sess=None,config={}, callback=None):
        """
    ...
    def traj_segment_generator(self, gail=True):
        ...
        while True:
            ...
            if gail:
                ...
                elif 'performance_matching' in self.config['shaping_mode']:
                    reward = self.discriminator.get_reward_reg(observation.reshape(1, -1), next_observation.reshape(1, -1), true_reward, self.sess)[0]
                    # disc_rew = self.discriminator.get_reward(observation.reshape(1, -1), next_observation.reshape(1, -1), self.sess)[0]
                    # reward = np.power(disc_rew * true_reward - true_reward, 2)
