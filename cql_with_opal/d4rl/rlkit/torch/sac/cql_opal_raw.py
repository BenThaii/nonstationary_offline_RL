...
class CQLTrainer(TorchTrainer):
	...
	def train_from_torch(self, batch):        
		...
		# min_qf1_loss = torch.logsumexp(cat_q1 / self.cql_temp, dim=1,).mean() * self.min_q_weight * self.cql_temp
		# min_qf2_loss = torch.logsumexp(cat_q2 / self.cql_temp, dim=1,).mean() * self.min_q_weight * self.cql_temp
		...
		if self.use_cql_alpha_tuning:
			...
			self.eval_statistics['min_qf1_loss_tau'] = (min_qf1_loss - self.cql_tau).item()
			self.eval_statistics['min_qf2_loss_tau'] = (min_qf2_loss - self.cql_tau).item()
			min_qf1_loss = cql_alpha * (min_qf1_loss - self.cql_tau)
			min_qf2_loss = cql_alpha * (min_qf2_loss - self.cql_tau)

			self.eval_statistics['min_q1_loss'] = min_qf1_loss.item()
			self.eval_statistics['min_q2_loss'] = min_qf2_loss.item()
			

			self.cql_alpha_optimizer.zero_grad()
			alpha_prime_loss = -0.5 * (min_qf1_loss + min_qf2_loss)
			alpha_prime_loss.backward(retain_graph=True)
			self.eval_statistics['cql_alpha_grad'] = self.log_cql_alpha.grad.item()
			self.cql_alpha_optimizer.step()


			# alpha_prime_loss_qf1 = -min_qf1_loss 
			# alpha_prime_loss_qf2 = -min_qf2_loss
			
			# self.cql_alpha_optimizer.zero_grad()
			# alpha_prime_loss_qf1.backward(retain_graph=True)
			# self.cql_alpha_optimizer.step()

			# self.cql_alpha_optimizer.zero_grad()
			# alpha_prime_loss_qf2.backward(retain_graph=True)
			# self.cql_alpha_optimizer.step()
