...
class CQLTrainer(TorchTrainer):
	...
	def train_from_torch(self, batch):        
		...
		#also use actions (below), where action is now action feature (in z-space)
		...
		# this is the latent primitive encoding z generated by the OPAL primitive encoder
			...
			# this must be the OPAL encoder, not the offline policy because we try to train the primitive policy to act like the behavior policy according to this latent z
