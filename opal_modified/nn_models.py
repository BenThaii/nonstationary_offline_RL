...
class LMP(nn.Module):
	...
	def calc_loss(self, state_traj, action_traj, is_cuda):
		...
		#Ben: for debuggin purposes
		mean_dist = (encoder_mean-prior_mean).abs().sum()
		encoder_mu_mean = encoder_mean.mean()
		encoder_mu_std = encoder_mean.std()
		prior_mu_mean = prior_mean.mean()
		prior_mu_std = prior_mean.std()
		return kl_loss, nll_loss, mean_dist, encoder_mu_mean, encoder_mu_std, prior_mu_mean, prior_mu_std
