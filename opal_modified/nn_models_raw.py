...
class FCNetwork(nn.Module):
    def __init__(self, inp_dim, hidden_dims, out_dim, act_fn=nn.ReLU()):
        ...
        layer_lst.append(nn.Linear(hidden_dim, out_dim))
...
class ARPolicyNetwork(nn.Module):
	...
	def forward(self, latent, state, deterministic=False):
		...
		if deterministic:
			...
			a_0 = torch.distributions.Uniform(self.low_act + l_0*self.bin_size, self.low_act + (l_0+1)*self.bin_size).sample()
	...
	def calc_log_prob(self, latent, state, action):
		...
		for i in range(1, self.act_dim):
			log_prob -= self.ce_loss(self.act_mod[i](torch.cat([state_d, self.lin_mod[i-1](action[:,:i])], dim=1)), l_action[:,i])
...
class LMP(nn.Module):
	...
	def create_encoder(self):
		self.state_encoder = FCNetwork(inp_dim=self.state_dim, hidden_dims=self.hidden_dims, out_dim=self.hidden_dims[-1], act_fn=self.act_fn)
		self.birnn_encoder = nn.GRU(self.hidden_dims[-1] + self.action_dim, self.hidden_dims[-1], self.rnn_layers, batch_first=True, bidirectional=True)
	...
	def calc_loss(self, state_traj, action_traj, is_cuda):
		...
		encoder_mean, encoder_std = self.forward_encoder(state_traj, action_traj)
